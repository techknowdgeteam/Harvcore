...
DEVELOPERS_MT5_DESTINATION_PATH = r"C:\xampp\htdocs\harvcore\developers_mt5"
...
DEVELOPERS = r"C:\xampp\htdocs\harvcore\harvox\invharv\usersdata\developers\developers.json"
...
def fetch_developers():
    """
    Fetch developers linked to this computer's IP via VPS / VPS host followers.

    - Only fetches developers whose ID appears in `vps.user_id` (matched by local IP)
      OR in `vps_hosts_followers.follower_id` where owner_id is the matched VPS user.
    ...
    # ── Diagnostic helper: time every DB call ──────────────────────────────
    def _timed_db_call(label, query, params=None, wait_hint=None):
        ...
        try:
            ...
            print(f"    ⏱️  [{label}] FAILED after {elapsed:.2f}s: {e}")
            raise
    ...
    # =========================================================================
    def find_developer_ids_for_this_computer(local_ip):
        developer_ids = set()
        ...
        vps_result, _ = _timed_db_call(
            "vps lookup", vps_query, [local_ip],
            wait_hint={'signal': 'js-display', 'timeout': 8, 'expect_empty': True},
        )

        if vps_result.get('status') == 'success':
            vps_rows = vps_result.get('results', [])
            if vps_rows and len(vps_rows) > 0:
                matched_vps_user_id = vps_rows[0].get('user_id')
                if matched_vps_user_id is not None:
                    matched_vps_user_id = int(matched_vps_user_id)
                    developer_ids.add(matched_vps_user_id)
                    identification_method = 'vps_ip'
                    print(f"  ✅ VPS match found! user_id = {matched_vps_user_id}")
                else:
                    print(f"  ⚠️ VPS row found but user_id is NULL")
            else:
        ...
        if matched_vps_user_id is not None:
            ...
            if followers_result.get('status') == 'success':
                ...
                for row in follower_rows:
                    fid = row.get('follower_id')
                    if fid is not None:
                        try:
                            developer_ids.add(int(fid))
        ...
        return sorted(developer_ids), identification_method, matched_vps_user_id

    # =========================================================================
    # STEP 2: Fetch developer details from `developers` table
    # =========================================================================
    def fetch_developers_from_table(developer_ids):
        dev_map = {}
        ...
        query = f"""
            SELECT id, email, broker, login, server, broker_password,
                   application_status, advertisement, created_at, updated_at,
                   terminal_path
            FROM developers
            WHERE id IN ({placeholders})
        """
        result, _ = _timed_db_call(
            "developers lookup", query, developer_ids,
            wait_hint={'signal': 'js-display', 'timeout': 8, 'expect_empty': True},
        )
        ...
        if result.get('status') == 'success':
            for row in result.get('results', []):
                ...
                if dev_id is not None:
                    dev_map[int(dev_id)] = row
        else:
            print(f"    ⚠️ developers query failed: {result.get('message')}")
    ...
    def fetch_selected_symbols_map(developer_ids):
        ...
        placeholders = ','.join(['%s'] * len(developer_ids))
        query = f"""
            SELECT userid, symbol, selected_timeframes
            FROM broker_symbols
        ...
        print(f"    🔎 Querying broker_symbols for {len(developer_ids)} developer ID(s)...")

        result, elapsed = _timed_db_call(
            "broker_symbols lookup", query, developer_ids,
            wait_hint={'signal': 'js-display', 'timeout': 8, 'expect_empty': True},
        )

        if not isinstance(result, dict):
            print(f"    ⚠️ broker_symbols: unexpected result type ({type(result).__name__}) — treating as empty")
            return symbols_map, timeframes_map

        if result.get('status') != 'success':
            print(f"    ⚠️ broker_symbols query failed: {result.get('message')}")
            return symbols_map, timeframes_map
        ...
        for row in rows:
            uid = row.get('userid')
            sym = row.get('symbol')
            tf_raw = row.get('selected_timeframes')
    ...
    # =========================================================================
    print("\n" + "="*70)
    print(f"  FETCHING DEVELOPERS (VPS-BASED IDENTIFICATION)")
    print("="*70)
    print(f"  Start Time  : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Output File : {DEVELOPERS}")
    print("-"*70)
    ...
    try:
        ...
        developer_ids, identification_method, matched_vps_user_id = \
            find_developer_ids_for_this_computer(local_ip)

        if not developer_ids:
            print(f"\n{'='*70}")
            print(f"  EXPORT SKIPPED - NO DEVELOPER IDS FOUND")
            print(f"{'='*70}")
            print(f"  Reason: No VPS row matches IP {local_ip}")
            print(f"{'='*70}")
            return

        print(f"\n  ✅ Total developer IDs to fetch: {len(developer_ids)}")
        ...
        if dev_map:
            print(f"  ✅ Found {len(dev_map)} developer record(s) in database")
        else:
        ...
        if symbols_map:
            ...
            print(f"  ✅ Selected symbols for {len(symbols_map)} developer(s) — {total_symbols} total symbol(s)")
        else:
            print(f"  ℹ️ No selected symbols found")

        if timeframes_map:
            total_tfs = sum(len(v) for v in timeframes_map.values())
            print(f"  ✅ Selected timeframes for {len(timeframes_map)} developer(s) — {total_tfs} total entry(ies)")
        else:
        ...
        for dev_id in developer_ids:
            row = dev_map.get(dev_id)
            if not row:
                print(f"    ⚠️ Developer ID {dev_id} not found in developers table — skipping")
                continue

            email = row.get('email') or ''
            fullname = email.split('@')[0] if email else ''
            ...
            record = {
                ...
                "fullname": fullname,
                "login": row.get('login'),
                "broker_password": row.get('broker_password'),
            ...
            all_records.append((dev_id, record))
            print(
        ...
        for idx, (dev_id, record) in enumerate(all_records):
            ...
            formatted_json = '\n'.join(indented_lines)
            content_parts.append(f'  "{dev_id}": {formatted_json}')

        content = ''.join(content_parts)
        safe_write_file(temp_path, 'a', content)
        ...
        if os.path.exists(temp_path):
            ...
            if file_size >= 4:
                print(f"  ✅ Valid temp file: {os.path.basename(temp_path)} ({file_size} bytes)")
                temp_file_valid = True
            else:
        ...
        if temp_file_valid:
            ...
            if move_temp_to_final(temp_path, DEVELOPERS):
                print(f"  ✅ Developers saved to: {DEVELOPERS}")
            else:
                print(f"  ❌ Failed to move temp file")
                return
        else:
            print(f"\n⚠️ [SKIPPED] Temp file invalid. Keeping existing file intact.")
            if os.path.exists(temp_path):
                try:
                    os.remove(temp_path)
                except:
        ...
        print(f"  👥 Developer IDs        : {len(developer_ids)}")
...
def fetch_investors():
    ...
    def _timed_db_call(label, query, params=None, wait_hint=None):
        """
        Run db.execute_query with timing.
        
        wait_hint (dict | None) is forwarded to db.execute_query so the
        Selenium wrapper can bail out the instant the JS page signals the
        query returned (via #sql-query-display / #execute-btn), rather than
        waiting for a result <table> that never appears for empty results.
        """
        import time as _time
        t0 = _time.perf_counter()
    ...
    # ── Diagnostic: dump a DB response so we know exactly what came back ──
    def _diag_dump(label, result):
        """
        Print the raw shape of a db.execute_query result so we can pinpoint
        where a failure occurs (status vs message vs results vs types).
        """
    ...
    def find_investor_ids_for_this_computer(local_ip):
        ...
        return sorted(investor_ids), identification_method, matched_vps_user_id
    ...
    def fetch_invested_with_map(investor_ids):
        ...
        if result.get('status') == 'success':
            for row in result.get('results', []):
                inv_id = row.get('investorid')
                dev_id = row.get('developerid')
                prog_name = row.get('program_name') or ''
                prog_id = row.get('programme_id')
    ...
    def fetch_accountmanagement_map(investor_developer_pairs):
        ...
        if result.get('status') == 'success':
            for row in result.get('results', []):
                inv_id = row.get('investorid')
                dev_id = row.get('developerid')
                if inv_id is not None and dev_id is not None:
                    am_map[(int(inv_id), int(dev_id))] = row
        else:
    ...
    def fetch_server_tier_limit():
        ...
        try:
            ...
            if result.get('status') == 'success' and result.get('results'):
                raw = result['results'][0].get('tier_limit')
    ...
    # =========================================================================
    # STEP 4: Build the accountmanagement object
    # =========================================================================
    SETTINGS_FIELDS = [
        'use_recent_highest_balance_as_current_balance',
        'symbols_grid_strategy',
        'enable_single_position_and_pending',
        'manage_grid_levels_count',
    ...
    JSON_LIKE_FIELDS = [
        ...
        'martingale_per_stage_drawdown_amount',
    ...
    def build_requirements_object(pi_row):
        ...
        return {
            'contract_duration': _dec(pi_row.get('contract_duration')),
            'minimum_investment_amount': _dec(pi_row.get('minimum_investment_amount')),
            'maximum_investment_amount': _dec(pi_row.get('maximum_investment_amount')),
            'investor_percentage': _dec(pi_row.get('investor_percentage')),
            'developer_percentage': _dec(pi_row.get('developer_percentage')),
            'invested_at': _dec(pi_row.get('invested_at')),
            'status': pi_row.get('status'),
        }
    ...
    try:
        ...
        investor_ids_to_fetch, identification_method, matched_vps_user_id = \
        ...
        # ---------------------------------------------------------------------
        # STEP 2: Build accountmanagement map
        # ---------------------------------------------------------------------
        ...
        # ── PRIMARY: SHOW COLUMNS (works without information_schema privileges) ──
        # ── and independent of DATABASE() binding ──
        ...
        if not columns:
            ...
            print(f"    1. SHOW COLUMNS FROM harvhub            → no usable rows")
            print(f"    2. information_schema.COLUMNS + DATABASE() → no usable rows")
            print(f"    3. SELECT * FROM harvhub LIMIT 1        → no usable row")
            print(f"  ℹ️ Check the [DIAG] dumps above for the raw responses.")
            print(f"  ℹ️ Common causes:")
            print(f"     - Table 'harvhub' does not exist in the connected schema")
            print(f"     - DB user lacks permission to read harvhub")
            print(f"     - Wrapper parser returned empty for a single-column SELECT")
            print(f"{'='*70}")
            return
        ...
        if count_result.get('status') == 'success' and count_result.get('results'):
            total_rows = int(count_result['results'][0].get('total') or 0)
        else:
...
def update_developers():
    ...
    def _serialize_timeframes(tf_value):
        if tf_value is None:
            return None
        if isinstance(tf_value, list):
            cleaned = [str(x).strip() for x in tf_value if str(x).strip()]
    ...
    try:
        ...
        dev_data = safe_read_file(DEVELOPERS)
        if not isinstance(dev_data, dict):
            print(f"   ❌ Developers file has invalid format (expected dict)")
            return

        total_devs = len(dev_data)
        print(f"   ✅ Loaded: {DEVELOPERS} ({total_devs:,} records)")

        if not dev_data:
            print(f"\n   ❌ No developers to process")
            return
        ...
        dev_columns = _schema('developers')
        ...
        existing_dev_ids = set()
        if existing_result.get('status') == 'success':
            for row in existing_result.get('results', []):
                v = row.get('id')
                if v is not None:
                    existing_dev_ids.add(str(v))
        print(f"   📊 Existing records in developers: {len(existing_dev_ids):,}")
        ...
        if existing_bs_result.get('status') == 'success':
            for row in existing_bs_result.get('results', []):
                uid = to_int_safe(row.get('userid'))
                sym = row.get('symbol')
                rid = to_int_safe(row.get('id'))
                if uid is None or sym is None:
                    continue
                sym = str(sym).strip()
                if not sym:
                    continue
                if rid is not None:
                    existing_bs_key_to_id[(uid, sym)] = rid
                existing_bs_by_user.setdefault(uid, set()).add(sym)
        print(f"   📊 Existing broker_symbols users: {len(existing_bs_by_user):,}")
        ...
        start_time = datetime.now()
        counters = {
            'dev_inserted': 0, 'dev_updated': 0, 'dev_failed': 0,
            'bs_users': 0,
            'bs_rows_updated': 0,
            'bs_rows_inserted': 0,
            'bs_rows_failed': 0,
        ...
        def flush_dev_batches():
            while dev_insert_rows:
                ...
                for rid, d in chunk:
                    ...
                    for c in all_cols:
                        if c == 'id':
                            continue
                        vals.append(d.get(c, 'NULL'))
                    value_groups.append("(" + ", ".join(vals) + ")")

                q = (f"INSERT INTO developers "
                     f"({', '.join('`'+c+'`' for c in all_cols)}) "
                     f"VALUES {', '.join(value_groups)}")
                r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 20})
                if r.get('status') == 'success':
                    counters['dev_inserted'] += len(chunk)
                    for rid, _ in chunk:
                        existing_dev_ids.add(str(rid))
                else:
            ...
            while dev_update_rows:
                ...
                cols = [c for c in DEV_FIELDS if c in col_set]
                if not cols:
                    continue
                ...
                for col in cols:
                    ...
                    case_parts.append(
                        f"`{col}` = CASE `id` {' '.join(whens)} END"
                    )
                q = (f"UPDATE developers SET {', '.join(case_parts)} "
                     f"WHERE `id` IN ({', '.join(str(i) for i in ids)})")
                r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 20})
                if r.get('status') == 'success':
                    counters['dev_updated'] += len(chunk)
                else:
                    counters['dev_failed'] += len(chunk)
        ...
        def flush_bs_batches():
            while bs_insert_rows:
                ...
                if has_timeframes_col:
                    ...
                    value_groups = []
                    for uid, sym_lit, _ in chunk:
                        value_groups.append(f"({uid}, {sym_lit})")
                    q = (f"INSERT INTO broker_symbols "
                         f"(`userid`, `symbol`) "
                         f"VALUES {', '.join(value_groups)}")
                ...
                r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 20})
                if r.get('status') == 'success':
                    counters['bs_rows_inserted'] += len(chunk)
                else:
            ...
            while bs_update_by_id:
                ...
                if not has_timeframes_col:
                    continue

                whens = []
                ids = []
                for row_id, tf_lit in chunk:
                    ids.append(str(row_id))
                    whens.append(f"WHEN {row_id} THEN {tf_lit}")

                q = (f"UPDATE broker_symbols "
                     f"SET `timeframes` = CASE `id` {' '.join(whens)} END "
                     f"WHERE `id` IN ({', '.join(ids)})")
                r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 20})
                if r.get('status') == 'success':
                    counters['bs_rows_updated'] += len(chunk)
                else:
                    counters['bs_rows_failed'] += len(chunk)
        ...
        for index, (dev_id, developer) in enumerate(dev_data.items(), 1):
            if index % 50 == 0 or index == total_devs:
                pct = (index / total_devs) * 100
                bar_len = 30
                filled = int(bar_len * index // total_devs)
                bar = '█' * filled + '░' * (bar_len - filled)
                print(f"  [{bar}] {pct:5.1f}% | {index:,}/{total_devs:,}", end='\r')
            ...
            for field in DEV_FIELDS:
                if field not in developer:
                    continue
                if field.lower() not in dev_columns:
                    dev_unmapped.add(field)
                    continue
                dev_fields[field] = sql_literal(developer[field])

            if not dev_fields:
                counters['dev_failed'] += 1
                continue

            if str(dev_id) in existing_dev_ids:
                dev_update_rows.append((userid, dev_fields))
            else:
            ...
            if isinstance(symbols_list, list) and bs_columns:
            ...
            if (len(dev_insert_rows) >= BATCH_SIZE or
                ...
                len(bs_update_by_id) >= BATCH_SIZE):
                flush_dev_batches()
                flush_bs_batches()
        ...
        print("="*70)
        print(f"\n  📊 developers:")
        print(f"     Inserted     : {counters['dev_inserted']:,}")
        print(f"     Updated      : {counters['dev_updated']:,}")
        print(f"     Failed       : {counters['dev_failed']:,}")
        if dev_unmapped:
            print(f"     Skipped flds : {len(dev_unmapped)} -> {sorted(dev_unmapped)}")
        print(f"\n  📊 broker_symbols:")
        print(f"     Users touched: {counters['bs_users']:,}")
        print(f"     Rows UPDATED : {counters['bs_rows_updated']:,}")
        print(f"     Rows INSERTED: {counters['bs_rows_inserted']:,}")
        print(f"     Rows FAILED  : {counters['bs_rows_failed']:,}")
...
def update_investors():
    ...
    source tables:
      ...
      - harvhub                 ← flat fields
      - accountmanagement       ← accountmanagement.{settings.*, <top-level>}
    ...
    try:
        ...
        am_columns        = _schema('accountmanagement');      print(f"   ✅ accountmanagement      : {len(am_columns)} cols")
        ...
        counters = {
            ...
            'am_inserted': 0, 'am_updated': 0, 'am_failed': 0,
        ...
        # --- harvhub / accountmanagement / investors_analytics buffers ---
        harvhub_updates = []   # list of (id_int, {col: literal})
        harvhub_inserts = []   # list of {col: literal, 'id': str}
        am_updates = []        # list of ((investorid, developerid), {col: literal})
        am_inserts = []        # list of {col: literal, 'investorid': str, 'developerid': str}
        ...
        def flush_accountmanagement():
            nonlocal am_updates, am_inserts
            if am_inserts:
                for grp in chunked(am_inserts, BATCH_SIZE):
                    col_set = set()
                    for d in grp: col_set.update(d.keys())
                    all_cols = ['investorid', 'developerid'] + [
                        c for c in sorted(col_set)
                        if c not in ('investorid', 'developerid')
                    ]
                    vgroups = []
                    for d in grp:
                        vals = [d.get(c, 'NULL') for c in all_cols]
                    ...
                    if update_cols:
                        ...
                        tail = "ON DUPLICATE KEY UPDATE `investorid` = `investorid`"
                    q = (f"INSERT INTO accountmanagement "
                         f"({', '.join(f'`{c}`' for c in all_cols)}) "
                         f"VALUES {', '.join(vgroups)} {tail}")
                    r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 25})
                    if r.get('status') == 'success':
                        counters['am_inserted'] += len(grp)
                    else:
                        counters['am_failed'] += len(grp)
                am_inserts = []

            if am_updates:
                for grp in chunked(am_updates, BATCH_SIZE):
                    # Update by (investorid, developerid) — batched CASE per column
                    col_set = set()
                    for _, d in grp: col_set.update(d.keys())
                    cols = sorted(col_set)
                    if not cols:
                        continue
                    # Match expression: (investorid, developerid) key
                    key_cases = []
                    keys = []
                    for (inv, dev), _ in grp:
                        keys.append((inv, dev))
                    # We can't do CASE on tuple easily; use id-free WHERE with OR
                    # But that's per-row.  Simpler: do one UPDATE per row for AM.
                    # (AM is small — one row per user.)
                    for (inv, dev), d in grp:
                        if not d:
                            continue
                        sets = ", ".join(f"`{c}` = {d[c]}" for c in d if c in cols)
                        q = (f"UPDATE accountmanagement SET {sets} "
                             f"WHERE investorid = {int(inv)} AND developerid = {int(dev)}")
                        r = db.execute_query(q, wait_hint={'signal': 'any-of', 'timeout': 15})
                        if r.get('status') == 'success':
                            counters['am_updated'] += 1
                        else:
                            counters['am_failed'] += 1
                am_updates = []
        ...
        for index, (investor_id, investor) in enumerate(merged_data.items(), 1):
            ...
            # ── (B) accountmanagement ──
            am_obj = investor.get('accountmanagement')
            if isinstance(am_obj, dict) and am_columns:
                dev_id = None
                for k in ('developerid', 'developerId', 'developer_id'):
                    v = am_obj.get(k)
                    if v is not None:
                        dev_id = to_int_safe(v)
                        if dev_id is not None: break
                if dev_id is None:
                    iw = investor.get('invested_with')
                    if isinstance(iw, str) and '_' in iw:
                        dev_id = to_int_safe(iw.split('_', 1)[0])
                if dev_id is not None:
                    merged_am = {}
                    sub = am_obj.get('settings')
                    if isinstance(sub, dict): merged_am.update(sub)
                    for k, v in am_obj.items():
                        if k == 'settings' or k in AM_TOP_EXCLUDED: continue
                        merged_am[k] = v

                    am_payload = {}
                    for field, value in merged_am.items():
                        if field.lower() not in am_columns:
                            am_unmapped.add(field); continue
                        lit_kv = sql_kv(field, value)
                        _, _, rhs = lit_kv.partition('=')
                        am_payload[field] = rhs.strip()

                    if am_payload:
                        # We let ON DUPLICATE KEY handle insert-vs-update.
                        ins = dict(am_payload)
                        ins['investorid'] = str(userid)
                        ins['developerid'] = str(dev_id)
                        am_inserts.append(ins)
            ...
            if (len(harvhub_inserts) >= BATCH_SIZE or
                ...
                len(am_inserts) >= BATCH_SIZE or
                len(ia_inserts) >= BATCH_SIZE):
                flush_harvhub()
                flush_accountmanagement()
        ...
        print(f"\n  📊 accountmanagement:")
        print(f"     Inserted/Upserted : {counters['am_inserted']:,}")
        print(f"     Updated           : {counters['am_updated']:,}")
        print(f"     Failed            : {counters['am_failed']:,}")
        print(f"     Skipped flds      : {len(am_unmapped)}")
        print(f"\n  📊 investors_analytics:")
...
def create_investor_mt5_files(inv_id=None):
    ...
    # =========================================================================
    # RULE 2: Folder exists → always verify + emit corrected values
    # =========================================================================
    if folder_exists:
        ...
        # Always emit Terminal_path if it differs from expected
        if not path_matches:
            update_data['Terminal_path'] = normalized_path
            print(f"   🔧 Terminal_path corrected")
        else:
            # Even if matching, we still emit it so the merge step records
            # the canonical value (idempotent). This is safe.
            update_data['Terminal_path'] = normalized_path
            print(f"   ✓ Terminal_path verified")

        # Always emit mt5_folder_name if it differs from expected
        ...
        # Status transition (pending → just-joined)
    ...
    # =========================================================================
    # RULE 3: Folder missing → create it, then emit fresh values
    # =========================================================================
...
def create_developer_mt5_files(dev_id=None):
    """
    Creates MT5 terminal folders for developers by copying from DEFAULT_MT5_PATH.

    Differences vs create_investor_mt5_files:
      - Reads from DEVELOPERS json (developers.json), not ALL_FETCHED_INVESTORS.
      - NO suspended-account check.
      - NO application_status logic — nothing is set/updated for status.
      - If the folder already exists, exit immediately with success.

    Args:
        dev_id: Required - specific developer ID to process (for multiprocessing)
    ...
    Returns:
        dict: {
            ...
            'success': bool,
            'created': bool,
            'deleted': bool,          # always False here (no deletion logic)
            'message': str,
            'updated_data': dict      # { dev_id: { terminal_path, mt5_folder_name } }
        }
    """
    ...
    # MUST have dev_id for multiprocessing
    if dev_id is None:
        return {
            ...
            'created': False,
            'deleted': False,
            'message': 'dev_id is required for multiprocessing',
            'updated_data': None
        }
    ...
    print(f"\n{'='*60}")
    print(f"📦 CREATE/MAINTAIN MT5 FILES (DEVELOPER) - ID: {dev_id}")
    print(f"{'='*60}")
    ...
    # Check if developers file exists
    if not os.path.exists(DEVELOPERS):
        msg = f"Developers file not found: {DEVELOPERS}"
        print(f" {msg}")
        result['message'] = msg
        return result

    # Load developers data (read-only)
    try:
        with open(DEVELOPERS, 'r', encoding='utf-8') as f:
            developers_data = json.load(f)
        print(f"📋 Loaded developers data")
    except Exception as e:
        msg = f"Error loading developers: {e}"
        print(f" {msg}")
        result['message'] = msg
        return result

    dev_id_str = str(dev_id)
    if dev_id_str not in developers_data:
        msg = f"Developer {dev_id} not found in data"
        print(f" {msg}")
        result['message'] = msg
        return result

    # Work on a copy of the developer data
    developer_data = developers_data[dev_id_str].copy()

    # Extract broker, id, and email for the folder name
    broker = (developer_data.get('broker') or '').strip()
    developer_id_value = (developer_data.get('id') or '').strip()
    email = (developer_data.get('email') or '').strip()

    if not broker or not developer_id_value:
        msg = f"Developer {dev_id} missing broker or id, skipping"
        print(f"🛑 {msg}")
        result['message'] = msg
        return result
    ...
    if email:
        ...
        safe_email = "no_email"
        print(f"🛑 ID:{dev_id} has no email address - using 'no_email' in folder name")

    # Create target paths (same format as investor folders)
    folder_name = f"MetaTrader 5 {safe_email} {developer_id_value} {broker}"
    target_folder = os.path.join(DEVELOPERS_MT5_DESTINATION_PATH, folder_name)
    target_exe = os.path.join(target_folder, "terminal64.exe")
    ...
    if folder_exists:
        print(f"✓ Folder exists: {folder_name}")
        print(f"   ℹ️ Exiting immediately (folder already present)")
        ...
        result['success'] = True
        result['created'] = False
        result['message'] = "Folder exists - terminal_path verified"
        result['updated_data'] = {dev_id_str: update_data}

        # Save individual result to temp file
        temp_result_file = os.path.join(tempfile.gettempdir(), f"create_dev_result_{dev_id}.json")
        try:
            with open(temp_result_file, 'w', encoding='utf-8') as f:
                json.dump(result, f, indent=2)
        except Exception as e:
            print(f"    ⚠️ Could not write temp result file: {e}")
...
def merge_developer_create_results():
    """
    Merge all individual developer MT5 folder-creation results from temp files
    back into the DEVELOPERS json file (developers.json).

    Reads temp files named: create_dev_result_*.json
    Writes into: DEVELOPERS
    """
    import os
    import json
    import tempfile
    ...
    print(f"\n{'='*60}")
    print(f"📦 MERGING DEVELOPER MT5 FOLDER CREATION RESULTS")
    print(f"{'='*60}")

    # Load current developers data
    if not os.path.exists(DEVELOPERS):
        print(f" Developers file not found: {DEVELOPERS}")
        return {'total_processed': 0, 'created': 0, 'deleted': 0, 'updated': 0, 'errors': 0}
    ...
    try:
        with open(DEVELOPERS, 'r', encoding='utf-8') as f:
            developers_data = json.load(f)
        print(f"📋 Loaded current developers data")
    except Exception as e:
        print(f" Error loading developers: {e}")
        return {'total_processed': 0, 'created': 0, 'deleted': 0, 'updated': 0, 'errors': 0}

    # Find all temp result files (developer-specific prefix)
    temp_dir = tempfile.gettempdir()
    result_files = glob.glob(os.path.join(temp_dir, "create_dev_result_*.json"))
    ...
    for result_file in result_files:
        try:
            ...
            if result.get('success') and result.get('updated_data'):
                ...
                if developer_id in developers_data:
                    new_data = updated_data.get(developer_id, {})
                    old_data = developers_data[developer_id]

                    updated = False
                    for field, value in new_data.items():
                        if field in old_data and value != old_data.get(field):
                            developers_data[developer_id][field] = value
                            updated = True
                            print(f"   🔄 Updated {field} for developer {developer_id}")
                        elif field not in old_data:
                            developers_data[developer_id][field] = value
                            updated = True
                            print(f"   ➕ Added {field} for developer {developer_id}")
                    ...
                    if updated or result.get('created') or result.get('deleted'):
                        print(f"✅ Merged update for developer {developer_id}: {result.get('message', '')[:60]}")
                else:
                    # If developer doesn't exist, add them
                    developers_data[developer_id] = updated_data.get(developer_id, {})
                    stats['created'] += 1
                    print(f"➕ Added new developer {developer_id}")
            else:
                stats['errors'] += 1
                print(f"🛑 Failed result for developer {result.get('developer_id', 'unknown')}: {result.get('message', 'No message')}")
    ...
    # Save merged data if there were changes
    if stats['total_processed'] > 0 and stats['errors'] < stats['total_processed']:
        backup_path = DEVELOPERS.replace('.json', '_backup.json')
        if not os.path.exists(backup_path):
            shutil.copy2(DEVELOPERS, backup_path)
            print(f"\n📦 Created backup: {backup_path}")

        with open(DEVELOPERS, 'w', encoding='utf-8') as f:
            json.dump(developers_data, f, indent=2)

        print(f"\n💾 Saved merged data to {DEVELOPERS}")
    else:
        print(f"\n🛑 No valid updates to save")

    # Print summary
    print(f"\n{'='*60}")
    print(f"📊 MERGE SUMMARY - DEVELOPER MT5 FOLDER CREATION")
    print(f"{'='*60}")
    print(f"   Total processed    : {stats['total_processed']}")
    print(f"   ✅ Created folders  : {stats['created']}")
    print(f"   🗑️  Deleted folders  : {stats['deleted']}")
...
def process_all_fetched_developers(dev_id):

    """
    WORKER FUNCTION: Handles the entire pipeline for ONE developer.

    ── DIFFERENCE FROM INVESTOR VERSION ──
      - NO MT5 initialization at all (no mt5.initialize, no login).
      - NO account info lookup.
      - NO balance / mode / account-type logic.
      - Just ensures the MT5 folder exists for this developer
        and records `terminal_path` / `mt5_folder_name` back into
        developers.json via the standard temp-file → merge flow.

    Args:
        dev_id: String developer ID (e.g., "1", "5", etc.)

    Returns:
        dict: status summary for this developer
    """

    dev_str_id = str(dev_id)

    print(f"\n[START] ⚙️ Registering and handling Developer ID: {dev_str_id}")
    ...
    # =====================================================================
    # SECTION: LOAD DEVELOPER DATA FIRST
    # =====================================================================
    developer_data = None
    ...
    try:
        if not os.path.exists(DEVELOPERS):
            msg = f"Developers file not found: {DEVELOPERS}"
            print(f"[ERROR] {msg}")
            developer_stats["skip_reason"] = msg
            return developer_stats

        with open(DEVELOPERS, 'r', encoding='utf-8') as f:
            all_developers = json.load(f)

        developer_data = all_developers.get(dev_str_id)

        if not developer_data:
            msg = f"Developer ID {dev_str_id} not found in DEVELOPERS"
            print(f"[ERROR] {msg}")
            developer_stats["skip_reason"] = msg
            return developer_stats

        print(f"✅ Found developer {dev_str_id} in DEVELOPERS")
        print(f"   Email  : {developer_data.get('email', '')}")
        print(f"   Broker : {developer_data.get('broker', '')}")
        print(f"   Server : {developer_data.get('server', '')}")
        print(f"   Existing terminal_path: {developer_data.get('terminal_path', 'NULL')}")

    except Exception as e:
        msg = f"Failed to load developer data: {str(e)}"
        print(f"[ERROR] {msg}")
        developer_stats["skip_reason"] = msg
        return developer_stats

    # =====================================================================
    # SECTION: RUN THE FILE-CREATION PIPELINE (NO MT5)
    # =====================================================================
    # NOTE: create_developer_mt5_files already:
    #   - checks if the folder exists → exits immediately with success
    #   - otherwise copies DEFAULT_MT5_PATH → destination
    #   - writes its own temp result file (create_dev_result_{id}.json)
    # We don't need to re-check anything. Just call it and read its result.
    try:
        result = create_developer_mt5_files(dev_id=dev_str_id)

        if result.get("success"):
            developer_stats["success"] = True
            developer_stats["created"] = bool(result.get("created", False))
            developer_stats["message"] = result.get("message", "")
            print(
                f"✅ Developer {dev_str_id} processed "
                f"({'created' if developer_stats['created'] else 'already existed'})"
            )
        else:
            developer_stats["skipped"] = True
            developer_stats["skip_reason"] = result.get("message", "Unknown error")
            print(f"⏭️  Developer {dev_str_id} skipped: {developer_stats['skip_reason']}")

    except Exception as e:
        msg = f"create_developer_mt5_files failed: {str(e)}"
        print(f"[ERROR] {msg}")
        developer_stats["skip_reason"] = msg
        return developer_stats
...
def main_once_developers():
    """
    ORCHESTRATOR (single-pass, no loop): processes ALL developers from
    developers.json by ensuring each has an MT5 folder + terminal_path.

    Flow:
      1. Load developer IDs from DEVELOPERS json.
      2. For each ID, spawn a worker via multiprocessing.
      3. Each worker calls create_developer_mt5_files (writes a temp result).
      4. After all workers finish, merge_developer_create_results() writes the
         terminal_path / mt5_folder_name back into developers.json.

    NOTE: There is NO MT5 initialization anywhere in this flow.
    """

    print("\n" + "="*60)
    print("="*60)
    print("  🧑‍💻 DEVELOPER MT5 FOLDER PIPELINE")
    print("  (file-creation only, NO MT5 initialization)")
    print("="*60 + "\n")

    # ─────────────────────────────────────────────────────────────
    # STEP 1: Load developer IDs
    # ─────────────────────────────────────────────────────────────
    developer_ids = []

    if not os.path.exists(DEVELOPERS):
        print(f"[ERROR] DEVELOPERS file not found: {DEVELOPERS}")
    ...
    try:
        ...
        developer_ids = list(developers_data.keys())
        print(f"✅ Loaded {len(developer_ids)} developers from {DEVELOPERS}")

    except Exception as e:
        print(f"[ERROR] Failed to load DEVELOPERS: {str(e)}")
        print("Single execution completed with error. Exiting...")
        return

    if not developer_ids:
        print(" ⏳ No developers found in DEVELOPERS. Nothing to do.")
        print("Single execution completed. Exiting...")
        return

    print(f"📋 Developer IDs: {developer_ids}")

    # ─────────────────────────────────────────────────────────────
    # STEP 2: Spawn one worker per developer (no capacity limits)
    # ─────────────────────────────────────────────────────────────
    cpu_cores = os.cpu_count() or 1
    available_ram_mb = psutil.virtual_memory().available / (1024 * 1024)

    print(f"\n🖥️  Hardware Profile -> Cores: {cpu_cores} | Free RAM: {available_ram_mb:.1f}MB")
    print(f"📊 Processing ALL {len(developer_ids)} developers without capacity restrictions...")

    print(f"\n--- Cycle Start: Spinning up context pool for {len(developer_ids)} workers ---")

    results = []
    try:
        with mp.Pool(processes=len(developer_ids)) as pool:
            jobs = []
            for dev_id in developer_ids:
                job = pool.apply_async(process_all_fetched_developers, args=(dev_id,))
                jobs.append(job)

            # Force synchronization: collect all results before closing pool
            results = [job.get() for job in jobs]

    except Exception as e:
        print(f"Critical Error in Developer Orchestrator: {e}")
        import traceback
        traceback.print_exc()
    ...
    # ─────────────────────────────────────────────────────────────
    print(f"\n--- Cycle Complete. Processed {len(results)} developers. ---")
    ...
    for r in results:
        if r.get("skip_reason"):
            print(f"  ⏭️  Developer {r.get('dev_id')}: {r.get('skip_reason')}")

    # ─────────────────────────────────────────────────────────────
    # STEP 4: Merge temp results back into developers.json
    # ─────────────────────────────────────────────────────────────
    merge_developer_create_results()
...
def process_all_fetched_investors(inv_id):
    ...
    FLOW:
      ...
      2. Pre-init validation: verify that Terminal_path belongs to THIS investor
         (folder name contains {id} and {broker}).
           - If valid → proceed to init.
           - If invalid → run create_investor_mt5_files so the JSON gets
             corrected, then abort this cycle (self-healing, no deadlock).
      3. Initialize MT5 by path only (no credentials).
      4. Verify account_info().login/server match the JSON record.
      5. Run get_investors_balance + verify_investors_balance.
    ...
    try:
        ...
        broker_cfg = {
            ...
            'Terminal_path': investor_data.get('Terminal_path', ''),
            'BROKER': investor_data.get('broker', ''),
            'ID': investor_data.get('id', ''),
            'EMAIL': investor_data.get('email', ''),
        }

        print(f"   Login       : {broker_cfg['LOGIN_ID']}")
        print(f"   Server      : {broker_cfg['SERVER']}")
        print(f"   Broker      : {broker_cfg['BROKER']}")
        print(f"   Terminal    : {broker_cfg['Terminal_path']}")
    ...
    # =====================================================================
    # PRE-INIT PATH VALIDATION
    # =====================================================================
    # Folder naming convention:
    #   MetaTrader 5 {safe_email} {id} {broker}
    # Path must contain BOTH {id} and {broker}. If either is missing, the
    # path is stale — we self-heal by calling create_investor_mt5_files()
    # so the JSON is fixed for the next cycle, then we abort this cycle.
    # ---------------------------------------------------------------------
    print(f"\n{'='*10} 🔍 PRE-INIT PATH VALIDATION FOR {inv_id} {'='*10}")

    configured_path = broker_cfg.get("Terminal_path", "") or ""
    expected_id = str(broker_cfg.get('ID') or inv_str_id).strip()
    expected_broker = str(broker_cfg.get('BROKER') or '').strip()

    path_valid = True

    if not configured_path or configured_path.upper() == "NULL":
        print(f"   ❌ Terminal_path is empty or NULL")
        path_valid = False
    else:
        folder_name = os.path.basename(os.path.dirname(configured_path))

        print(f"   Folder name : {folder_name}")
        print(f"   Expect id   : {expected_id}")
        print(f"   Expect brkr : {expected_broker}")

        # id must appear as a token in the folder name
        id_present = False
        if expected_id:
            id_present = (
                f" {expected_id} " in f" {folder_name} "
                or folder_name.endswith(f" {expected_id}")
            )

        # broker must appear (case-insensitive)
        broker_present = False
        if expected_broker:
            broker_present = expected_broker.lower() in folder_name.lower()

        if not id_present:
            print(f"   ❌ Folder name does NOT contain investor id '{expected_id}'")
            path_valid = False
        if not broker_present:
            print(f"   ❌ Folder name does NOT contain broker '{expected_broker}'")
            path_valid = False

        if path_valid:
            print(f"   ✅ Path matches this investor's id + broker")
        else:
            print(f"   ⚠️ STALE PATH detected")

    # =====================================================================
    # SELF-HEAL: if path is invalid, run create_investor_mt5_files so the
    # JSON gets corrected for the next cycle, then abort this cycle.
    # =====================================================================
    if not path_valid:
        print(f"\n{'='*10} 🛠️ SELF-HEAL: correcting investor {inv_id} record {'='*10}")
        try:
            heal = create_investor_mt5_files(inv_id=inv_id)
            if heal.get('success'):
                print(f"   ✅ create_investor_mt5_files succeeded: {heal.get('message', '')}")
                print(f"      updated_data = {heal.get('updated_data')}")
                print(f"      → merge_create_results() will persist this correction")
            else:
                print(f"   ⚠️ create_investor_mt5_files reported failure: {heal.get('message', '')}")
        except Exception as e:
            print(f"   ⚠️ create_investor_mt5_files raised: {e}")

        account_stats["skip_reason"] = (
            "Stale Terminal_path — attempted self-heal via create_investor_mt5_files"
        )
        return account_stats

    # =====================================================================
    # MT5 INITIALIZATION BY PATH ONLY (no credentials)
    # =====================================================================
    ...
    # =====================================================================
    # GET ACCOUNT INFO — AND VERIFY IT'S THE RIGHT ACCOUNT
    # =====================================================================
    ...
    if acc is None:
        ...
        account_stats["skip_reason"] = "No account info after init"
        try:
            mt5.shutdown()
        except Exception:
            pass
    ...
    # ── POST-INIT VERIFICATION: MT5 must have attached to THIS investor's account ──
    try:
        expected_login_int = int(str(broker_cfg.get('LOGIN_ID')).strip())
    except (ValueError, TypeError):
        print(f"🛑 Invalid LOGIN_ID in JSON: {broker_cfg.get('LOGIN_ID')!r}")
        account_stats["skip_reason"] = "Invalid login id"
        try:
            mt5.shutdown()
        except Exception:
            pass
    ...
    if acc.login != expected_login_int:
        print(f"🛑 ACCOUNT MISMATCH — aborting to avoid operating on the wrong account")
        print(f"   Terminal path : {terminal_path}")
        print(f"   Expected login: {expected_login_int}")
        print(f"   Actual login  : {acc.login}")
        print(f"   Actual server : {acc.server}")
        print(f"   ℹ️ The terminal at this path is logged into a DIFFERENT account.")
        print(f"      Log into the correct account on this terminal manually.")
        account_stats["skip_reason"] = (
            f"Account mismatch: terminal has {acc.login}, expected {expected_login_int}"
        )
        try:
            mt5.shutdown()
        except Exception:
    ...
    # Verify server string (case-insensitive)
    expected_server = str(broker_cfg.get('SERVER') or '').strip()
    if expected_server and acc.server and acc.server.lower() != expected_server.lower():
        print(f"🛑 SERVER MISMATCH — terminal is on a different server")
        print(f"   Expected server: {expected_server}")
        print(f"   Actual server  : {acc.server}")
        account_stats["skip_reason"] = (
            f"Server mismatch: terminal on '{acc.server}', expected '{expected_server}'"
        )
        try:
            mt5.shutdown()
        except Exception:
    ...
    print(f"   ✅ Account identity matches JSON (login={acc.login}, server={acc.server})")
    ...
    if acc.trade_mode == mt5.ACCOUNT_TRADE_MODE_REAL:
        ...
        if any(indicator in server_upper for indicator in
               ["DEMO", "STAGE", "TEST", "PRELIVE", "SIMULATION"]):
            is_real = False
    ...
    # Extract demo permission policy
    ...
    # =====================================================================
    # SUCCESS — MT5 CONNECTED TO THE CORRECT ACCOUNT
    # =====================================================================
    ...
    # create_investor_mt5_files stays right here, unchanged position
