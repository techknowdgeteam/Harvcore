...
SUSPENDED_ACCOUNTS = r"C:\xampp\htdocs\harvcore\harvox\harvhub\suspended_accounts.json"
...
def update_tables_streaming(batch_size=5000):
    ...
    try:
        ...
        # Check for updated_harvhub_investors.json first
...
def merge_balance_results():
    ...
    if updated_count > 0:
        ...
        # Also update updated_harvhub_investors.json with valid credentials
...
def merge_verify_results():
    ...
    if updated_count > 0:
        ...
        # Update updated_harvhub_investors.json with verified investors
        updated_investors_data = {}
        for investor_id, investor_data in investors_data.items():
...
def place_orders_parallel():
    """
    ORCHESTRATOR: Processes all investors from fetched_harvhub_investors.json
    ...
    try:
        with open(FETCHED_INVESTORS, 'r', encoding='utf-8') as f:
            investors_data = json.load(f)
        print(f"📋 Found {len(investors_data)} investors in fetched_harvhub_investors.json")
    except Exception as e:
        print(f"Error loading investors: {e}")
        return False
...
def place_orders_parallel_loop():
    ...
    """
    print(f"🚀 Starting Perpetual Trading Loop (using fetched_harvhub_investors.json)...")
